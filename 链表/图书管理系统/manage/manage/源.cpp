...
图书馆中有下列书籍
Fielding Henry
	*Pasquin - checked out to Chapman Car1
...
Brown Jim has the following books
	...
	*Murdoch Iris,The Red and Green
...
void status() {
	...
	cout << "\nThe following peole are using the library:\n\n";
	for (i = 'A'; i <= 'Z'; i++) {
		if (!people[i].empty())
			cout << people[i];
	}
...
int main() {
	while(true)
		switch (menu()) {
			...
			case 4:status();	   break;
			case 5:return 0;
			default: cout << "Wrong optoin, try again: ";
		}
	return 0;
}